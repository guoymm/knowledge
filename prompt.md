...
## 2026-04-26 — Card-Authenticity 关键字段选取逻辑

**类型**: 知识

各品类"关键字段"（矛盾即 serious）：餐饮=价格+推荐菜；TTD=营业时间+价格+
地理位置；住宿=地理位置+酒店服务；购物=品牌/门店概览+特色商品+价格+地理位置；
本地生活=特色服务+价格+地理位置。选取逻辑：这些字段的矛盾直接影响用户核心
决策（预算/能否去/能否进门）；关键字段以外的矛盾（如装修风格描述不一致）不算
serious。

## 2026-05-08 — demand judge FN 修法：结构性区分，不引入具体 case

**类型**: 经验

demand judge 漏判（FN）改 prompt 时，用"primary subject vs secondary
mention"这类**结构性区分**，绝不写进具体 query/POI 名（过拟合）。例：lastfeed
主体是创作者个人生活、POI 名只是评论里观众猜测时 → 非 LS，即使 query 精确命中
POI 名。并显式反驳模型常用开脱理由（"query 命中 POI 名"不构成 LS 证据）。
